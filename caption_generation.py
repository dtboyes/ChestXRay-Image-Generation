...
class decoder_block(tf.keras.layers.Layer):
  ...
  def __init__(self,vocab_size, embedding_dim, max_pad, dense_dim ,name = "token_decoder"):
    ...
    self.embedding = Embedding(input_dim = vocab_size+1,
                                ...
                                mask_zero=True, 
                                name = 'token_decoder_embedding'
                              )
    self.LSTM = GRU(units=self.dense_dim,
                    # return_sequences=True,
                    return_state=True,
                    name = 'token_decoder_LSTM'
                    )
    self.attention = global_attention(dense_dim = dense_dim)
    self.concat = Concatenate(axis=-1)
    self.dense = Dense(dense_dim,name = 'token_decoder_embedding_dense',activation = 'relu')
...
class decoder(tf.keras.Model):
  ...
  def call(self,encoder_output,caption):#,decoder_h,decoder_c): #caption : (None,max_pad), encoder_output: (None,dense_dim)
    ...
    for timestep in range(self.max_pad): #iterating through all timesteps ie through max_pad
      output,decoder_h,attention_weights = self.token_decoder(caption[:,timestep:timestep+1], encoder_output, decoder_h)
...
def greedy_search_predict(image1,image2,model,tokenizer,input_size = (224,224)):
  ...
  for i in range(max_pad):
    ...
    output,decoder_h,_ = model.get_layer('decoder').token_decoder(caption,enc_op,decoder_h)#,decoder_c) decoder_c,
